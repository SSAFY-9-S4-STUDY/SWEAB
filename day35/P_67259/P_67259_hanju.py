...
def solution(board):
    ...
    N = len(board)
    # 각 칸 당 최저 
    min_cost_arr = [[[float("inf")] * 4 for _1 in range(N)] for _2 in range(N)]  

    # 2. bfs와 dp를 위한 초기값 세팅
    # 시작 지점 코스트 -500(시작지점에서 이동할 때 코너가 있다고 판단해버리기 때문)
    min_cost_arr[0][0] = [-500] * 4
    ...
    while Q:
        ...
        for d in range(4):
            ...
            total_cost = min_cost_arr[row][col][dir] + cost
            # 현재까지의 비용이 이동 지점에 기록된 비용보다 적다면 계속해서 탐색
            if total_cost < min_cost_arr[nrow][ncol][d]:
                min_cost_arr[nrow][ncol][d] = total_cost
                if nrow != N-1 or ncol != N-1:
                    Q.append((nrow, ncol, d))
            
    return min(min_cost_arr[-1][-1])
