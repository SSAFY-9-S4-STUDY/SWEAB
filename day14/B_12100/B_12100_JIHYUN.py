...
def dfs(cnt, arr):
    ...
    dfs(cnt + 1, move(deepcopy(arr), 0))  # 0: 위
    dfs(cnt + 1, move(deepcopy(arr), 1))  # 1: 아래
    dfs(cnt + 1, move(deepcopy(arr), 2))  # 2: 왼쪽
    dfs(cnt + 1, move(deepcopy(arr), 3))  # 3: 오른쪽
